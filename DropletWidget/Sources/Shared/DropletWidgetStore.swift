...
final class DropletWidgetStore {
    ...
    private enum Key {
        ...
        static let legacyWidgetGradientEnabled = "widgetGradientEnabled"
    ...
    var gradientEnabled: Bool {
        get {
            if defaults.object(forKey: Key.gradientEnabled) != nil {
                return defaults.bool(forKey: Key.gradientEnabled)
            }
            if defaults.object(forKey: Key.legacyWidgetGradientEnabled) != nil {
                return defaults.bool(forKey: Key.legacyWidgetGradientEnabled)
            }
            return true
        }
        set { defaults.set(newValue, forKey: Key.gradientEnabled) }
    }
    ...
    func syncGradient(isEnabled: Bool) {
    ...
    private func reloadWidgets() {
        #if canImport(WidgetKit)
        WidgetCenter.shared.reloadTimelines(ofKind: DropletWidgetKind.today)
        WidgetCenter.shared.reloadTimelines(ofKind: DropletWidgetKind.streakGarden)
        WidgetCenter.shared.reloadTimelines(ofKind: DropletWidgetKind.timerBucket)
        #endif
    }
}
